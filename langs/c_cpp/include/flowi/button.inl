typedef struct FlButtonApi {
  ...
  bool (*regular_size)(struct FlInternalData *priv, FlString label,
                       FlVec2 size);
  bool (*small)(struct FlInternalData *priv, FlString label);
  bool (*invisible)(struct FlInternalData *priv, FlString label, FlVec2 size,
                    FlButtonFlags flags);
  ...
  bool (*image_with_text)(struct FlInternalData *priv, FlImage image,
                          FlString label);
} FlButtonApi;
...
bool fl_button_regular_size_impl(struct FlInternalData *priv, FlString label,
                                 FlVec2 size);
bool fl_button_small_impl(struct FlInternalData *priv, FlString label);
bool fl_button_invisible_impl(struct FlInternalData *priv, FlString label,
                              FlVec2 size, FlButtonFlags flags);
bool fl_button_check_box_impl(struct FlInternalData *priv, FlString label,
                              bool *state);
bool fl_button_radio_impl(struct FlInternalData *priv, FlString label,
                          bool state);
void fl_button_bullet_impl(struct FlInternalData *priv);
bool fl_button_image_with_text_impl(struct FlInternalData *priv, FlImage image,
                                    FlString label);
#endif
...
#else
  return (g_flowi_button_api->regular_size)(g_flowi_button_api->priv, label_,
                                            size);
#endif
}
...
// button.
FL_INLINE bool fl_button_invisible(const char *label, FlVec2 size,
                                   FlButtonFlags flags) {
  FlString label_ = fl_cstr_to_flstring(label);
#ifdef FLOWI_STATIC
  return fl_button_invisible_impl(g_flowi_button_api->priv, label_, size,
                                  flags);
#else
  return (g_flowi_button_api->invisible)(g_flowi_button_api->priv, label_, size,
                                         flags);
#endif
}
...
#else
  return (g_flowi_button_api->check_box)(g_flowi_button_api->priv, label_,
                                         state);
#endif
}
...
FL_INLINE bool fl_button_image_with_text(FlImage image, const char *label) {
  FlString label_ = fl_cstr_to_flstring(label);
#ifdef FLOWI_STATIC
  return fl_button_image_with_text_impl(g_flowi_button_api->priv, image,
                                        label_);
#else
  return (g_flowi_button_api->image_with_text)(g_flowi_button_api->priv, image,
                                               label_);
#endif
}
