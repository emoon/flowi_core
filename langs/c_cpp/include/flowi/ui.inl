typedef struct FlUiApi {
  ...
  void (*image_with_size)(struct FlInternalData *priv, FlImage image,
                          FlVec2 size);
  void (*set_pos)(struct FlInternalData *priv, FlVec2 pos);
  FlRect (*get_last_widget_size)(struct FlInternalData *priv, FlVec2 pos);
  bool (*push_button_with_icon)(struct FlInternalData *priv, FlString text,
                                FlImage image, FlVec2 text_pos,
                                float image_scale);
...
void fl_ui_image_with_size_impl(struct FlInternalData *priv, FlImage image,
                                FlVec2 size);
...
bool fl_ui_push_button_with_icon_impl(struct FlInternalData *priv,
                                      FlString text, FlImage image,
                                      FlVec2 text_pos, float image_scale);
...
// Draw image. Images can be created with [Image::create_from_file] and
// [Image::create_from_memory]
...
// Set position for the next ui-element (this is used when [LayoutMode::Manual]
// is used)
...
FL_INLINE bool fl_ui_push_button_with_icon(const char *text, FlImage image,
                                           FlVec2 text_pos, float image_scale) {
  FlString text_ = fl_cstr_to_flstring(text);
#ifdef FLOWI_STATIC
  return fl_ui_push_button_with_icon_impl(g_flowi_ui_api->priv, text_, image,
                                          text_pos, image_scale);
#else
  return (g_flowi_ui_api->push_button_with_icon)(g_flowi_ui_api->priv, text_,
                                                 image, text_pos, image_scale);
#endif
}
