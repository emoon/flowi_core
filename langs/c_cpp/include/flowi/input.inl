...
FL_INLINE void fl_input_add_key_analog_event(FlKey key, bool down,
                                             float value) {
#ifdef FLOWI_STATIC
  fl_input_add_key_analog_event_impl(g_flowi_input_api->priv, key, down, value);
#else
  (g_flowi_input_api->add_key_analog_event)(g_flowi_input_api->priv, key, down,
                                            value);
#endif
}
...
#else
  (g_flowi_input_api->add_mouse_button_event)(g_flowi_input_api->priv, button,
                                              down);
#endif
}
