typedef struct FlMenuApi {
  ...
  bool (*item_ex)(struct FlInternalData *priv, FlString label,
                  FlString shortcut, bool selected, bool enabled);
  bool (*item_toggle)(struct FlInternalData *priv, FlString label,
                      FlString shortcut, bool *selected, bool enabled);
} FlMenuApi;
...
bool fl_menu_begin_impl(struct FlInternalData *priv, FlString label,
                        bool enabled);
void fl_menu_end_impl(struct FlInternalData *priv);
bool fl_menu_item_impl(struct FlInternalData *priv, FlString label);
bool fl_menu_item_ex_impl(struct FlInternalData *priv, FlString label,
                          FlString shortcut, bool selected, bool enabled);
bool fl_menu_item_toggle_impl(struct FlInternalData *priv, FlString label,
                              FlString shortcut, bool *selected, bool enabled);
#endif
...
FL_INLINE bool fl_menu_item_ex(const char *label, const char *shortcut,
                               bool selected, bool enabled) {
  FlString label_ = fl_cstr_to_flstring(label);
  FlString shortcut_ = fl_cstr_to_flstring(shortcut);
#ifdef FLOWI_STATIC
  return fl_menu_item_ex_impl(g_flowi_menu_api->priv, label_, shortcut_,
                              selected, enabled);
#else
  return (g_flowi_menu_api->item_ex)(g_flowi_menu_api->priv, label_, shortcut_,
                                     selected, enabled);
#endif
}

// return true when activated + toggle selected
FL_INLINE bool fl_menu_item_toggle(const char *label, const char *shortcut,
                                   bool *selected, bool enabled) {
  FlString label_ = fl_cstr_to_flstring(label);
  FlString shortcut_ = fl_cstr_to_flstring(shortcut);
#ifdef FLOWI_STATIC
  return fl_menu_item_toggle_impl(g_flowi_menu_api->priv, label_, shortcut_,
                                  selected, enabled);
#else
  return (g_flowi_menu_api->item_toggle)(g_flowi_menu_api->priv, label_,
                                         shortcut_, selected, enabled);
#endif
}
