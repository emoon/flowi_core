typedef struct FlCursorApi {
  ...
  void (*same_line)(struct FlInternalData *priv, float offset_from_start_x,
                    float spacing);
...
void fl_cursor_same_line_impl(struct FlInternalData *priv,
...
float fl_cursor_get_text_line_height_with_spacing_impl(
    struct FlInternalData *priv);
...
#ifdef FLOWI_STATIC
  fl_cursor_same_line_impl(g_flowi_cursor_api->priv, offset_from_start_x,
                           spacing);
#else
  (g_flowi_cursor_api->same_line)(g_flowi_cursor_api->priv, offset_from_start_x,
                                  spacing);
#endif
}
...
// Add a dummy item of given size. Unlike widgets.invisible_button(), dummmy()
...
// generally top-left == GetMainViewport()->Pos == (0,0) in single viewport
// mode, and bottom-right == GetMainViewport()->Pos+Size == io.DisplaySize in
...
#ifdef FLOWI_STATIC
  return fl_cursor_get_text_line_height_with_spacing_impl(
      g_flowi_cursor_api->priv);
#else
  return (g_flowi_cursor_api->get_text_line_height_with_spacing)(
      g_flowi_cursor_api->priv);
#endif
}
...
#else
  return (g_flowi_cursor_api->get_frame_height_with_spacing)(
      g_flowi_cursor_api->priv);
#endif
}
