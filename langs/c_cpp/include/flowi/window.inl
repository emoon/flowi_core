typedef struct FlWindowApi {
  ...
  bool (*begin)(struct FlInternalData *priv, FlString name,
                FlWindowFlags flags);
  void (*end)(struct FlInternalData *priv);
  bool (*begin_child)(struct FlInternalData *priv, FlString id, FlVec2 size,
                      bool border, FlWindowFlags flags);
...
bool fl_window_begin_impl(struct FlInternalData *priv, FlString name,
                          FlWindowFlags flags);
void fl_window_end_impl(struct FlInternalData *priv);
bool fl_window_begin_child_impl(struct FlInternalData *priv, FlString id,
                                FlVec2 size, bool border, FlWindowFlags flags);
...
bool fl_window_is_focused_impl(struct FlInternalData *priv,
                               FlFocusedFlags flags);
bool fl_window_is_hovered_impl(struct FlInternalData *priv,
                               FlHoveredFlags flags);
...
FL_INLINE bool fl_window_begin_child(const char *id, FlVec2 size, bool border,
                                     FlWindowFlags flags) {
  FlString id_ = fl_cstr_to_flstring(id);
#ifdef FLOWI_STATIC
  return fl_window_begin_child_impl(g_flowi_window_api->priv, id_, size, border,
                                    flags);
#else
  return (g_flowi_window_api->begin_child)(g_flowi_window_api->priv, id_, size,
                                           border, flags);
#endif
}
