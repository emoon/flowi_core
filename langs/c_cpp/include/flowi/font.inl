...
<<<<<<< HEAD
  ...
  FlFont (*load)(struct FlInternalData *priv, FlString filename,
                 uint32_t font_size);
  FlFont (*load_with_range)(struct FlInternalData *priv, FlString filename,
                            uint32_t font_size, uint16_t glyph_range_start,
                            uint16_t glyph_range_end);
  FlFont (*load_from_memory)(struct FlInternalData *priv, FlString name,
...
#ifdef FLOWI_STATIC
FlFont fl_font_load_impl(struct FlInternalData *priv, FlString filename,
                         uint32_t font_size);
FlFont fl_font_load_with_range_impl(struct FlInternalData *priv,
                                    FlString filename, uint32_t font_size,
                                    uint16_t glyph_range_start,
                                    uint16_t glyph_range_end);
FlFont fl_font_load_from_memory_impl(struct FlInternalData *priv, FlString name,
                                     uint8_t *data, uint32_t data_size,
                                     uint32_t font_size);
...
// pre-generated
FL_INLINE FlFont fl_font_load_with_range(const char *filename,
                                         uint32_t font_size,
                                         uint16_t glyph_range_start,
                                         uint16_t glyph_range_end) {
  FlString filename_ = fl_cstr_to_flstring(filename);
#ifdef FLOWI_STATIC
  return fl_font_load_with_range_impl(g_flowi_font_api->priv, filename_,
                                      font_size, glyph_range_start,
                                      glyph_range_end);
#else
  return (g_flowi_font_api->load_with_range)(g_flowi_font_api->priv, filename_,
                                             font_size, glyph_range_start,
                                             glyph_range_end);
#endif
}
...
FL_INLINE FlFont fl_font_load_from_memory(const char *name, uint8_t *data,
                                          uint32_t data_size,
                                          uint32_t font_size) {
  FlString name_ = fl_cstr_to_flstring(name);
#ifdef FLOWI_STATIC
  return fl_font_load_from_memory_impl(g_flowi_font_api->priv, name_, data,
                                       data_size, font_size);
#else
  return (g_flowi_font_api->load_from_memory)(g_flowi_font_api->priv, name_,
                                              data, data_size, font_size);
=======
    struct FlInternalData* priv;
