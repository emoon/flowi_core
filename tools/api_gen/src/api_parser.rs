...
impl ApiParser {
            ...
            match entry.as_rule() {
                Rule::field => {
                    ...
                    if field.as_rule() == Rule::enum_type {
                        entries.push(Self::get_enum(&doc_comments, &mut current_value, field));
                        doc_comments.clear();
                    }
                }
    ...
    fn get_enum(
        ...
        match value {
            EnumValue::None => {
                value = EnumValue::Value(current_value.to_string());
                *current_value += 1;
            }

            EnumValue::Value(ref v) => {
    ...
    pub fn second_pass(api_defs: &mut [ApiDef]) {
        ...
        for api_def in api_defs.iter() {
            api_def.structs
                .iter().chain(api_def.handle_structs.iter())
                .for_each(|s| 
            {
